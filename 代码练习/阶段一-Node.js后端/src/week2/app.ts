...
// CORS 白名单：从环境变量 CORS_ORIGINS 读取，逗号分隔（如 http://localhost:5173,https://myapp.com）
...
export function createApp() {
    ...
    // 404 兜底：所有路由都匹配不到时走这里（必须在错误处理之前）
    app.use((req: Request, res: Response) => {
        res.status(404).json({ message: '接口不存在', data: null })
    })
