...
describe('task接口集成测试', () => { //描述,相当于文件夹
    it('测试GET /tasks/list 列表接口', async () => {//当前接口测试用例的描述
    ...
    it('POST /tasks/create 创建任务接口', async () => {
        const token = await loginToken(`tester_${Date.now()}`)
        const res = await request(app).post('/tasks/create').set('Authorization', `Bearer ${token}`).send({ title: '标题', completed: false, priority: 'low' })
        expect(res.status).toBe(201)
        expect(res.body).toBeInstanceOf(Object)
    })
    it('PUT /tasks/update/:id 修改', async () => {
        const token = await loginToken(`tester_${Date.now()}`)
        const createRes = await request(app).post('/tasks/create').set('Authorization', `Bearer ${token}`).send({ title: '标题', completed: false, priority: 'low' })
        const res = await request(app).put(`/tasks/update/${createRes.body.id}`).set('Authorization', `Bearer ${token}`).send({ title: '标题', completed: false, priority: 'low' })
        expect(res.status).toBe(200)
        expect(res.body).toBeInstanceOf(Object)
    })
    it('DELETE /tasks/delete/:id 删除', async () => {
        ...
        const token = await loginToken(`tester_${Date.now()}`)
        const createRes = await request(app).post('/tasks/create').set('Authorization', `Bearer ${token}`).send({ title: '标题', completed: false, priority: 'low' })
        const res = await request(app).delete(`/tasks/delete/${createRes.body.id}`).set('Authorization', `Bearer ${token}`)
