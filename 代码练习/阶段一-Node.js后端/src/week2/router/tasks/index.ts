import { Router ,type Express, type Request, type Response, type NextFunction } from 'express'
import { task, listTasks, findTask,  createTask, updateTask, deleteTask, type Task } from '../../data/tasks/index.js'

import { createTaskSchema, listTasksSchema, taskIdSchema, updateTaskSchema } from '../../schemas/tasks/index.js'
export const taskRouter = Router()

//列表
taskRouter.get('/list', async (req, res, next) => {
    try {
        const result = listTasksSchema.safeParse(req.query)
        if (!result.success) {

            return res.status(500).json({ message: '查询参数不合法', data: result.error.flatten() })
        }
        const data = await listTasks(result.data)
        res.json(data)
    } catch (error) {
        next(error)
    }
})

//详情
taskRouter.get('/detail/:id', async (req, res, next) => {
    try {
        const result = taskIdSchema.safeParse(req.params.id)
        if (!result.success) { 

            return res.json({
                message: "id 必须是正整数",
                data: result.error.flatten()
            })
        }
        const data = await findTask(Number(req.params.id))

        if (!data) { 
            return res.status(500).json({ message: '任务不存在',data:{} })
        }
        return res.json(data)
    } catch (error) {
        next(error)
    }
})

//新增
taskRouter.post('/create', async (req, res, next) => {
    try {

        const result = createTaskSchema.safeParse(req.body)

        if (!result.success) {
            return res.status(500).json({ message: '请求体不合法', data: result.error.flatten() })
         }

        const task = await createTask(result.data)
        res.status(201).json(task)
    } catch (error) {
        next(error)
    }
})

//修改
taskRouter.put('/update/:id', async (req, res, next) => {
    try {

        const id = taskIdSchema.safeParse(req.params.id)
        if (!id.success) {
            return res.status(500).json({ message: 'id 必须是正整数', data: id.error.flatten() })
        }
        const result = updateTaskSchema.safeParse(req.body)

        if (!result.success) { 
            return  res.status(500).json({
                message: '请求体不合法',
                data: result.error.flatten()
            })
        }
        const task = await updateTask(id.data, result.data)
        if (!task) {
            return res.status(500).json({ message: '任务不存在',data:{} })
        }
        return res.json(task)

    } catch (error) {
        next(error)
    }
})

//删除
taskRouter.delete('/delete/:id', async (req, res, next) => {
    try {

        const result = taskIdSchema.safeParse(req.params.id)
        if (!result.success) { 
            return res.status(500).json({
                message: 'id 必须是正整数',
                data: result.error.flatten()
            })
        }
        const task = await deleteTask(result.data)
        if (!task) { 
            return res.status(500).json({
                message: '任务不存在',
                data: {}
            })
        }
          
        return res.json(task)
    } catch (error) {
        next(error)
    }
})